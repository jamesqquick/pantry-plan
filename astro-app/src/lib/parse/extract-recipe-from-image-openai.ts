 ...
 * Workers-adapted: uses ArrayBuffer + btoa instead of Node Buffer.
 * Caller must validate image (type, size) and auth.
 */
...
/** Convert ArrayBuffer to base64 string (Workers-compatible, no Node Buffer). */
...
/**
 * Extract recipe structure from an image using OpenAI Vision.
 * @param imageData - Image file content as ArrayBuffer.
 * @param mimeType - Image MIME type (e.g. image/jpeg, image/png, image/webp).
 * @param apiKey - OpenAI API key from Workers env.
 */
export async function extractRecipeFromImageOpenAI(
  ...
  try {
    ...
    if (!title)
      throw new Error("Could not read recipe title from image.");
