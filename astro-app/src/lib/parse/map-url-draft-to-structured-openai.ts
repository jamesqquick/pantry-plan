 ...
 * and structured ingredient rows.
 *
 * Workers-adapted: no `process.env`, no `server-only`. Caller passes apiKey.
 */
...
/**
 * Call OpenAI to map a URL-parsed draft + per-line candidates into a full recipe
 * and structured ingredient rows. Validates output against Zod schema and verifies
 * ingredientId is in the allowed candidates for each line.
 */
export async function mapUrlDraftToStructured(
  ...
  | {
      ok: true;
      recipe: UrlToStructuredRecipeResponse;
  ...
  for (const row of data.structuredIngredients) {
    ...
    if (row.ingredientId) {
      if (allowedIds && allowedIds.has(row.ingredientId)) {
        ...
        ingredientName =
          candidateNameById.get(row.ingredientId) ?? undefined;
      }
