/**
 * LLM fallback for ingredient mapping: sends unmapped lines + catalog to OpenAI,
 * returns Map<originalIndex, LlmSuggestion>.
 *
 ...
 * On parse/validation/API failure returns empty Map.
 */
export async function suggestMappingsWithLLM(
  ...
  if (!key || unmappedLines.length === 0 || catalog.length === 0) {
    return new Map();
  }
