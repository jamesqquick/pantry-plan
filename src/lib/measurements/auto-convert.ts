...
const RISKY_FLAGS = ["to taste", "garnish", "for garnish"] as const;
...
/** Ingredients where weight/volume conversion is approximate. */
...
/**
 * Compute normalized weightGrams + conversion metadata for a recipe ingredient line.
...
export function autoConvert(params: {
  ...
  if (isVolumeUnit(unit)) {
    ...
    if (gramsPerCup != null) {
      ...
      if (cups != null) result.weightGrams = cups * gramsPerCup;
    }
  }
  ...
  if (result.weightGrams != null) {
    ...
    if (isApproximateIngredient(ingredient)) {
      ...
      result.conversionNotes =
        "Original line contains packed/heaping/sifted etc.; conversion is approximate.";
    } else {
